...
echo "Starting server with Lightsail on port ${PORT}..."
"$BINARY" \
  --lightsail-port $PORT \
  --s3-port 14601 \
  --sns-port 14602 \
  ...
  --ecs-port 14701 \
  --eks-port 14702 \
  --apprunner-port 14703 \
  --batch-port 14704 \
  --elasticbeanstalk-port 14705 \
  --outposts-port 14706 \
...
echo "Running Lightsail integration tests..."

echo "Basic smoke test for Lightsail"
# Tests would go here for each operation
PASS=$((PASS + 1))
TESTS+=("PASS  Lightsail server started")

echo ""
echo "══════════════════════════════════════════════"
echo "  Lightsail Integration Test Results"
echo "══════════════════════════════════════════════"

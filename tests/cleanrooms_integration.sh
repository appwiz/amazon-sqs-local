...
echo "Starting server with CleanRooms on port ${PORT}..."
"$BINARY" \
  --cleanrooms-port $PORT \
  --s3-port 14601 \
  --sns-port 14602 \
  --sqs-port 14603 \
  --dynamodb-port 14604 \
...
echo "Running CleanRooms integration tests..."

echo "Basic smoke test for CleanRooms"
# Tests would go here for each operation
PASS=$((PASS + 1))
TESTS+=("PASS  CleanRooms server started")

echo ""
echo "══════════════════════════════════════════════"
echo "  CleanRooms Integration Test Results"
echo "══════════════════════════════════════════════"

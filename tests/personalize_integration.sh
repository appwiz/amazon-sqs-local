...
echo "Starting server with Personalize on port ${PORT}..."
"$BINARY" \
  --personalize-port $PORT \
  --s3-port 14601 \
  --sns-port 14602 \
  --sqs-port 14603 \
  --dynamodb-port 14604 \
...
echo "Running Personalize integration tests..."

echo "Basic smoke test for Personalize"
# Tests would go here for each operation
PASS=$((PASS + 1))
TESTS+=("PASS  Personalize server started")

echo ""
echo "══════════════════════════════════════════════"
echo "  Personalize Integration Test Results"
echo "══════════════════════════════════════════════"

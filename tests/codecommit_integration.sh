...
echo "Starting server with CodeCommit on port ${PORT}..."
"$BINARY" \
  --codecommit-port $PORT \
  --s3-port 14601 \
  --sns-port 14602 \
  --sqs-port 14603 \
  --dynamodb-port 14604 \
...
echo "Running CodeCommit integration tests..."

echo "Basic smoke test for CodeCommit"
# Tests would go here for each operation
PASS=$((PASS + 1))
TESTS+=("PASS  CodeCommit server started")

echo ""
echo "══════════════════════════════════════════════"
echo "  CodeCommit Integration Test Results"
echo "══════════════════════════════════════════════"

...
echo "Starting server with CodeBuild on port ${PORT}..."
"$BINARY" \
  --codebuild-port $PORT \
  --s3-port 14601 \
  --sns-port 14602 \
...
echo "Running CodeBuild integration tests..."

echo "Basic smoke test for CodeBuild"
# Tests would go here for each operation
PASS=$((PASS + 1))
TESTS+=("PASS  CodeBuild server started")

echo ""
echo "══════════════════════════════════════════════"
echo "  CodeBuild Integration Test Results"
echo "══════════════════════════════════════════════"

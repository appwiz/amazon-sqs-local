...
echo "Starting server with CodeArtifact on port ${PORT}..."
"$BINARY" \
  --codeartifact-port $PORT \
  --s3-port 14601 \
  --sns-port 14602 \
  --sqs-port 14603 \
  --dynamodb-port 14604 \
...
echo "Running CodeArtifact integration tests..."

echo "Basic smoke test for CodeArtifact"
# Tests would go here for each operation
PASS=$((PASS + 1))
TESTS+=("PASS  CodeArtifact server started")

echo ""
echo "══════════════════════════════════════════════"
echo "  CodeArtifact Integration Test Results"
echo "══════════════════════════════════════════════"

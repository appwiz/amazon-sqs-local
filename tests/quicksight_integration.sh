...
echo "Starting server with QuickSight on port ${PORT}..."
"$BINARY" \
  --quicksight-port $PORT \
  --s3-port 14601 \
  --sns-port 14602 \
  --sqs-port 14603 \
  --dynamodb-port 14604 \
...
echo "Running QuickSight integration tests..."

echo "Basic smoke test for QuickSight"
# Tests would go here for each operation
PASS=$((PASS + 1))
TESTS+=("PASS  QuickSight server started")

echo ""
echo "══════════════════════════════════════════════"
echo "  QuickSight Integration Test Results"
echo "══════════════════════════════════════════════"

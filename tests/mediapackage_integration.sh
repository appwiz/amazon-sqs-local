...
echo "Starting server with MediaPackage on port ${PORT}..."
"$BINARY" \
  --mediapackage-port $PORT \
  --s3-port 14601 \
  --sns-port 14602 \
  --sqs-port 14603 \
  --dynamodb-port 14604 \
...
echo "Running MediaPackage integration tests..."

echo "Basic smoke test for MediaPackage"
# Tests would go here for each operation
PASS=$((PASS + 1))
TESTS+=("PASS  MediaPackage server started")

echo ""
echo "══════════════════════════════════════════════"
echo "  MediaPackage Integration Test Results"
echo "══════════════════════════════════════════════"

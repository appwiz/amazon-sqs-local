...
echo "Starting server with CloudFront on port ${PORT}..."
"$BINARY" \
  --cloudfront-port $PORT \
  --s3-port 14601 \
  --sns-port 14602 \
  --sqs-port 14603 \
  --dynamodb-port 14604 \
...
echo "Running CloudFront integration tests..."

echo "Basic smoke test for CloudFront"
# Tests would go here for each operation
PASS=$((PASS + 1))
TESTS+=("PASS  CloudFront server started")

echo ""
echo "══════════════════════════════════════════════"
echo "  CloudFront Integration Test Results"
echo "══════════════════════════════════════════════"

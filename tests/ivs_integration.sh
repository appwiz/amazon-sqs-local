...
echo "Starting server with IVS on port ${PORT}..."
"$BINARY" \
  --ivs-port $PORT \
  --s3-port 14601 \
  --sns-port 14602 \
...
echo "Running IVS integration tests..."

echo "Basic smoke test for IVS"
# Tests would go here for each operation
PASS=$((PASS + 1))
TESTS+=("PASS  IVS server started")

echo ""
echo "══════════════════════════════════════════════"
echo "  IVS Integration Test Results"
echo "══════════════════════════════════════════════"

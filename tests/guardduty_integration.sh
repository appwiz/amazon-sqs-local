...
echo "Starting server with GuardDuty on port ${PORT}..."
"$BINARY" \
  --guardduty-port $PORT \
  --s3-port 14601 \
  --sns-port 14602 \
...
echo "Running GuardDuty integration tests..."

echo "Basic smoke test for GuardDuty"
# Tests would go here for each operation
PASS=$((PASS + 1))
TESTS+=("PASS  GuardDuty server started")

echo ""
echo "══════════════════════════════════════════════"
echo "  GuardDuty Integration Test Results"
echo "══════════════════════════════════════════════"

...
echo "Starting server with StorageGateway on port ${PORT}..."
"$BINARY" \
  --storagegateway-port $PORT \
  --s3-port 14601 \
  --sns-port 14602 \
  --sqs-port 14603 \
  --dynamodb-port 14604 \
...
echo "Running StorageGateway integration tests..."

echo "Basic smoke test for StorageGateway"
# Tests would go here for each operation
PASS=$((PASS + 1))
TESTS+=("PASS  StorageGateway server started")

echo ""
echo "══════════════════════════════════════════════"
echo "  StorageGateway Integration Test Results"
echo "══════════════════════════════════════════════"

...
echo "Starting server with WorkSpaces on port ${PORT}..."
"$BINARY" \
  --workspaces-port $PORT \
  --s3-port 14601 \
  --sns-port 14602 \
  --sqs-port 14603 \
  --dynamodb-port 14604 \
...
echo "Running WorkSpaces integration tests..."

echo "Basic smoke test for WorkSpaces"
# Tests would go here for each operation
PASS=$((PASS + 1))
TESTS+=("PASS  WorkSpaces server started")

echo ""
echo "══════════════════════════════════════════════"
echo "  WorkSpaces Integration Test Results"
echo "══════════════════════════════════════════════"

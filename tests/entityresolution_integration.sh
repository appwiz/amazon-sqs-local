...
echo "Starting server with EntityResolution on port ${PORT}..."
"$BINARY" \
  --entityresolution-port $PORT \
  --s3-port 14601 \
  --sns-port 14602 \
  --sqs-port 14603 \
  --dynamodb-port 14604 \
...
echo "Running EntityResolution integration tests..."

echo "Basic smoke test for EntityResolution"
# Tests would go here for each operation
PASS=$((PASS + 1))
TESTS+=("PASS  EntityResolution server started")

echo ""
echo "══════════════════════════════════════════════"
echo "  EntityResolution Integration Test Results"
echo "══════════════════════════════════════════════"

...
echo "Starting server with DevOpsGuru on port ${PORT}..."
"$BINARY" \
  --devopsguru-port $PORT \
  --s3-port 14601 \
  --sns-port 14602 \
  --sqs-port 14603 \
  --dynamodb-port 14604 \
...
echo "Running DevOpsGuru integration tests..."

echo "Basic smoke test for DevOpsGuru"
# Tests would go here for each operation
PASS=$((PASS + 1))
TESTS+=("PASS  DevOpsGuru server started")

echo ""
echo "══════════════════════════════════════════════"
echo "  DevOpsGuru Integration Test Results"
echo "══════════════════════════════════════════════"

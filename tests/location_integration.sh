...
echo "Starting server with Location on port ${PORT}..."
"$BINARY" \
  --location-port $PORT \
  --s3-port 14601 \
  --sns-port 14602 \
...
echo "Running Location integration tests..."

echo "Basic smoke test for Location"
# Tests would go here for each operation
PASS=$((PASS + 1))
TESTS+=("PASS  Location server started")

...
use super::error::QbusinessError;
use super::types::*;

#[allow(dead_code)]
struct QbusinessStateInner {
...
#[allow(dead_code)]
pub struct QbusinessState {
    inner: Arc<Mutex<QbusinessStateInner>>,
}
...
impl QbusinessState {
    ...
    pub fn new(account_id: String, region: String) -> Self {
        QbusinessState {
            inner: Arc::new(Mutex::new(QbusinessStateInner {
    ...
    #[allow(dead_code)]
    pub async fn create_application(&self, req: CreateApplicationRequest) -> Result<ApplicationDetail, QbusinessError> {
        ...
        if state.applications.contains_key(&name) {
            return Err(QbusinessError::ResourceAlreadyExistsException(format!("Application {} already exists", name)));
        }
        let arn = format!("arn:aws:qbusiness:{}:{}:applications/{}", state.region, state.account_id, name);
    ...
    #[allow(dead_code)]
    pub async fn get_application(&self, name: &str) -> Result<ApplicationDetail, QbusinessError> {
        let state = self.inner.lock().await;
        let stored = state.applications.get(name)
            .ok_or_else(|| QbusinessError::ResourceNotFoundException(format!("Application {} not found", name)))?;
    ...
    #[allow(dead_code)]
    pub async fn list_applications(&self) -> Result<ListApplicationsResponse, QbusinessError> {
        let state = self.inner.lock().await;
    ...
    #[allow(dead_code)]
    pub async fn delete_application(&self, name: &str) -> Result<(), QbusinessError> {
        let mut state = self.inner.lock().await;
...
mod tests {
    ...
    async fn test_new_state() {
        let _state = QbusinessState::new("123456789012".to_string(), "us-east-1".to_string());
    }
    #[tokio::test]
    async fn test_create_application() {
        let state = QbusinessState::new("123456789012".to_string(), "us-east-1".to_string());

mod _types {
use serde::{Deserialize, Serialize};

#[derive(Debug, Deserialize, Clone, Default)]
#[allow(dead_code)]
pub struct CreateRepositoryRequest {
    #[serde(rename = "RepositoryName")]
    pub repository_name: String,
    #[serde(rename = "Tags")]
    #[serde(default)]
    pub tags: Option<std::collections::HashMap<String, String>>,
}

#[derive(Debug, Serialize)]
#[allow(dead_code)]
pub struct CreateRepositoryResponse {
    #[serde(rename = "RepositoryArn")]
    #[serde(skip_serializing_if = "Option::is_none")]
    pub repository_arn: Option<String>,
    #[serde(rename = "RepositoryName")]
    #[serde(skip_serializing_if = "Option::is_none")]
    pub repository_name: Option<String>,
}

#[derive(Debug, Deserialize, Clone, Default)]
#[allow(dead_code)]
pub struct DescribeRepositoryRequest {
    #[serde(rename = "RepositoryName")]
    pub repository_name: Option<String>,
    #[serde(rename = "RepositoryArn")]
    pub repository_arn: Option<String>,
}

#[derive(Debug, Serialize, Clone)]
#[allow(dead_code)]
pub struct RepositoryDetail {
    #[serde(rename = "RepositoryName")]
    pub repository_name: String,
    #[serde(rename = "RepositoryArn")]
    pub repository_arn: String,
    #[serde(rename = "Status")]
    pub status: String,
    #[serde(rename = "Tags")]
    #[serde(skip_serializing_if = "Option::is_none")]
    pub tags: Option<std::collections::HashMap<String, String>>,
}

#[derive(Debug, Serialize)]
#[allow(dead_code)]
pub struct DescribeRepositoryResponse {
    #[serde(rename = "Repository")]
    pub repository: RepositoryDetail,
}

#[derive(Debug, Deserialize, Clone, Default)]
#[allow(dead_code)]
pub struct ListRepositorysRequest {
    #[serde(rename = "MaxResults")]
    pub max_results: Option<usize>,
    #[serde(rename = "NextToken")]
    pub next_token: Option<String>,
}

#[derive(Debug, Serialize)]
#[allow(dead_code)]
pub struct ListRepositorysResponse {
    #[serde(rename = "Repositorys")]
    #[serde(skip_serializing_if = "Option::is_none")]
    pub repositorys: Option<Vec<RepositoryDetail>>,
    #[serde(rename = "NextToken")]
    #[serde(skip_serializing_if = "Option::is_none")]
    pub next_token: Option<String>,
}

#[derive(Debug, Deserialize, Clone, Default)]
#[allow(dead_code)]
pub struct DeleteRepositoryRequest {
    #[serde(rename = "RepositoryName")]
    pub repository_name: Option<String>,
    #[serde(rename = "RepositoryArn")]
    pub repository_arn: Option<String>,
}

}
pub use _types::*;

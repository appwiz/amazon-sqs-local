...
async fn handle_request(
    State(state): State<Arc<ECRState>>,
    headers: HeaderMap,
    body: Bytes,
) -> Result<axum::response::Response, ECRError> {
    let target = headers
        .get("x-amz-target")
        .and_then(|v| v.to_str().ok())
        .ok_or_else(|| ECRError::InvalidAction("Missing X-Amz-Target header".into()))?;

    let action = target
        .strip_prefix("AmazonEC2ContainerRegistry_V20150921.")
        .ok_or_else(|| ECRError::InvalidAction(format!("Invalid target: {target}")))?;
    ...
    match action {
        ...
        _ => Err(ECRError::InvalidAction(format!("Unknown action: {action}"))),
    }
}

pub fn create_router(state: Arc<ECRState>) -> Router {
    Router::new()
...
mod tests {
    ...
    async fn test_missing_target_header() {
        let state = Arc::new(ECRState::new("123456789012".to_string(), "us-east-1".to_string()));
    ...
    async fn test_unknown_action() {
        ...
        let req = Request::builder()
            ...
            .uri("/")
            .header("content-type", "application/x-amz-json-1.1")
            .header("x-amz-target", "AmazonEC2ContainerRegistry_V20150921.FakeAction")
            .body(Body::from("{}"))
            .unwrap();
        let resp = app.oneshot(req).await.unwrap();
        assert_eq!(resp.status(), StatusCode::BAD_REQUEST);
    ...
    async fn test_describerepository_requires_params() {
        ...
        let req = Request::builder()
            ...
            .uri("/")
            .header("content-type", "application/x-amz-json-1.1")
            .header("x-amz-target", "AmazonEC2ContainerRegistry_V20150921.DescribeRepository")
            .body(Body::from("{}"))
            .unwrap();
        let resp = app.oneshot(req).await.unwrap();
    ...
    async fn test_listrepositorys_action() {
        ...
        let req = Request::builder()
            ...
            .uri("/")
            .header("content-type", "application/x-amz-json-1.1")
            .header("x-amz-target", "AmazonEC2ContainerRegistry_V20150921.ListRepositorys")
            .body(Body::from("{}"))
            .unwrap();
        let resp = app.oneshot(req).await.unwrap();

...
pub struct MacieState {
    inner: Arc<Mutex<MacieStateInner>>,
}
...
impl MacieState {
    ...
    pub fn new(account_id: String, region: String) -> Self {
        MacieState {
            inner: Arc::new(Mutex::new(MacieStateInner {
    ...
    #[allow(dead_code)]
    pub async fn create_finding(&self, req: CreateFindingRequest) -> Result<FindingDetail, MacieError> {
        let mut state = self.inner.lock().await;
        let name = req.name.or(req.name_pascal).unwrap_or_else(|| uuid::Uuid::new_v4().to_string());
        if state.findings.contains_key(&name) {
            return Err(MacieError::ResourceAlreadyExistsException(format!("Finding {} already exists", name)));
        }
        let arn = format!("arn:aws:macie2:{}:{}:findings/{}", state.region, state.account_id, name);
    ...
    #[allow(dead_code)]
    pub async fn get_finding(&self, name: &str) -> Result<FindingDetail, MacieError> {
        let state = self.inner.lock().await;
        let stored = state.findings.get(name)
            .ok_or_else(|| MacieError::ResourceNotFoundException(format!("Finding {} not found", name)))?;
        Ok(FindingDetail {
    ...
    #[allow(dead_code)]
    pub async fn list_findings(&self) -> Result<ListFindingsResponse, MacieError> {
        let state = self.inner.lock().await;
    ...
    #[allow(dead_code)]
    pub async fn delete_finding(&self, name: &str) -> Result<(), MacieError> {
        let mut state = self.inner.lock().await;
...
mod tests {
    ...
    async fn test_new_state() {
        let _state = MacieState::new("123456789012".to_string(), "us-east-1".to_string());
    }
    #[tokio::test]
    async fn test_create_finding() {
        let state = MacieState::new("123456789012".to_string(), "us-east-1".to_string());

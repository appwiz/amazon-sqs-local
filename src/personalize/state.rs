...
use super::error::PersonalizeError;
use super::types::*;

#[allow(dead_code)]
struct PersonalizeStateInner {
...
pub struct PersonalizeState {
    inner: Arc<Mutex<PersonalizeStateInner>>,
}
...
impl PersonalizeState {
    ...
    pub fn new(account_id: String, region: String) -> Self {
        PersonalizeState {
            inner: Arc::new(Mutex::new(PersonalizeStateInner {
    ...
    #[allow(dead_code)]
    pub async fn create_dataset(&self, req: CreateDatasetRequest) -> Result<CreateDatasetResponse, PersonalizeError> {
        let mut state = self.inner.lock().await;
        let name = req.dataset_name.clone();
        if state.datasets.contains_key(&name) {
            return Err(PersonalizeError::ResourceAlreadyExistsException(format!("Dataset {} already exists", name)));
        }
        let arn = format!("arn:aws:personalize:{}:{}:datasets/{}", state.region, state.account_id, name);
    ...
    #[allow(dead_code)]
    pub async fn describe_dataset(&self, req: DescribeDatasetRequest) -> Result<DescribeDatasetResponse, PersonalizeError> {
        let state = self.inner.lock().await;
        let name = req.dataset_name.or_else(|| req.dataset_arn.as_ref().and_then(|a| a.rsplit('/').next().map(|s| s.to_string())))
            .ok_or_else(|| PersonalizeError::ValidationException("Name or ARN required".to_string()))?;
        let stored = state.datasets.get(&name)
            .ok_or_else(|| PersonalizeError::ResourceNotFoundException(format!("Dataset {} not found", name)))?;
    ...
    #[allow(dead_code)]
    pub async fn list_datasets(&self, _req: ListDatasetsRequest) -> Result<ListDatasetsResponse, PersonalizeError> {
        let state = self.inner.lock().await;
    ...
    #[allow(dead_code)]
    pub async fn delete_dataset(&self, req: DeleteDatasetRequest) -> Result<(), PersonalizeError> {
        let mut state = self.inner.lock().await;
        let name = req.dataset_name.or_else(|| req.dataset_arn.as_ref().and_then(|a| a.rsplit('/').next().map(|s| s.to_string())))
            .ok_or_else(|| PersonalizeError::ValidationException("Name or ARN required".to_string()))?;
...
mod tests {
    ...
    async fn test_new_state() {
        let _state = PersonalizeState::new("123456789012".to_string(), "us-east-1".to_string());
    }
    #[tokio::test]
    async fn test_create_dataset() {
        let state = PersonalizeState::new("123456789012".to_string(), "us-east-1".to_string());

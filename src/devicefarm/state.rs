...
pub struct DevicefarmState {
    inner: Arc<Mutex<DevicefarmStateInner>>,
}
...
impl DevicefarmState {
    ...
    pub fn new(account_id: String, region: String) -> Self {
        DevicefarmState {
            inner: Arc::new(Mutex::new(DevicefarmStateInner {
    ...
    #[allow(dead_code)]
    pub async fn create_project(&self, req: CreateProjectRequest) -> Result<CreateProjectResponse, DevicefarmError> {
        let mut state = self.inner.lock().await;
        let name = req.project_name.clone();
        if state.projects.contains_key(&name) {
            return Err(DevicefarmError::ResourceAlreadyExistsException(format!("Project {} already exists", name)));
        }
        let arn = format!("arn:aws:devicefarm:{}:{}:projects/{}", state.region, state.account_id, name);
    ...
    #[allow(dead_code)]
    pub async fn describe_project(&self, req: DescribeProjectRequest) -> Result<DescribeProjectResponse, DevicefarmError> {
        let state = self.inner.lock().await;
        let name = req.project_name.or_else(|| req.project_arn.as_ref().and_then(|a| a.rsplit('/').next().map(|s| s.to_string())))
            .ok_or_else(|| DevicefarmError::ValidationException("Name or ARN required".to_string()))?;
        let stored = state.projects.get(&name)
            .ok_or_else(|| DevicefarmError::ResourceNotFoundException(format!("Project {} not found", name)))?;
    ...
    #[allow(dead_code)]
    pub async fn list_projects(&self, _req: ListProjectsRequest) -> Result<ListProjectsResponse, DevicefarmError> {
        let state = self.inner.lock().await;
    ...
    #[allow(dead_code)]
    pub async fn delete_project(&self, req: DeleteProjectRequest) -> Result<(), DevicefarmError> {
        let mut state = self.inner.lock().await;
        let name = req.project_name.or_else(|| req.project_arn.as_ref().and_then(|a| a.rsplit('/').next().map(|s| s.to_string())))
            .ok_or_else(|| DevicefarmError::ValidationException("Name or ARN required".to_string()))?;
...
mod tests {
    ...
    async fn test_new_state() {
        let _state = DevicefarmState::new("123456789012".to_string(), "us-east-1".to_string());
    }
    #[tokio::test]
    async fn test_create_project() {
        let state = DevicefarmState::new("123456789012".to_string(), "us-east-1".to_string());

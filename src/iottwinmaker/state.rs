...
use super::error::IottwinmakerError;
use super::types::*;

#[allow(dead_code)]
struct IottwinmakerStateInner {
...
#[allow(dead_code)]
pub struct IottwinmakerState {
    inner: Arc<Mutex<IottwinmakerStateInner>>,
}

impl IottwinmakerState {
    #[allow(dead_code)]
    pub fn new(account_id: String, region: String) -> Self {
        IottwinmakerState {
            inner: Arc::new(Mutex::new(IottwinmakerStateInner {
    ...
    #[allow(dead_code)]
    pub async fn create_workspace(&self, req: CreateWorkspaceRequest) -> Result<WorkspaceDetail, IottwinmakerError> {
        ...
        if state.workspaces.contains_key(&name) {
            return Err(IottwinmakerError::ResourceAlreadyExistsException(format!("Workspace {} already exists", name)));
        }
        let arn = format!("arn:aws:iottwinmaker:{}:{}:workspaces/{}", state.region, state.account_id, name);
    ...
    #[allow(dead_code)]
    pub async fn get_workspace(&self, name: &str) -> Result<WorkspaceDetail, IottwinmakerError> {
        let state = self.inner.lock().await;
        let stored = state.workspaces.get(name)
            .ok_or_else(|| IottwinmakerError::ResourceNotFoundException(format!("Workspace {} not found", name)))?;
        Ok(WorkspaceDetail {
    ...
    #[allow(dead_code)]
    pub async fn list_workspaces(&self) -> Result<ListWorkspacesResponse, IottwinmakerError> {
        let state = self.inner.lock().await;
    ...
    #[allow(dead_code)]
    pub async fn delete_workspace(&self, name: &str) -> Result<(), IottwinmakerError> {
        let mut state = self.inner.lock().await;
...
mod tests {
    ...
    async fn test_new_state() {
        let _state = IottwinmakerState::new("123456789012".to_string(), "us-east-1".to_string());
    }
    #[tokio::test]
    async fn test_create_workspace() {
        let state = IottwinmakerState::new("123456789012".to_string(), "us-east-1".to_string());

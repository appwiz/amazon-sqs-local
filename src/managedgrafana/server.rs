...
use super::state::ManagedgrafanaState;
use super::types::*;

async fn create_workspace_handler(
    State(state): State<Arc<ManagedgrafanaState>>,
    Json(req): Json<CreateWorkspaceRequest>,
) -> Result<axum::response::Response, ManagedgrafanaError> {
...
pub fn create_router(state: Arc<ManagedgrafanaState>) -> Router {
    Router::new()
...
mod tests {
    ...
    async fn test_list_endpoint() {
        let state = Arc::new(ManagedgrafanaState::new("123456789012".to_string(), "us-east-1".to_string()));

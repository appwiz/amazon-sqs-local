...
use super::error::ManagedgrafanaError;
use super::types::*;

#[allow(dead_code)]
struct ManagedgrafanaStateInner {
...
pub struct ManagedgrafanaState {
    inner: Arc<Mutex<ManagedgrafanaStateInner>>,
}
...
impl ManagedgrafanaState {
    ...
    pub fn new(account_id: String, region: String) -> Self {
        ManagedgrafanaState {
            inner: Arc::new(Mutex::new(ManagedgrafanaStateInner {
    ...
    #[allow(dead_code)]
    pub async fn create_workspace(&self, req: CreateWorkspaceRequest) -> Result<WorkspaceDetail, ManagedgrafanaError> {
        ...
        if state.workspaces.contains_key(&name) {
            return Err(ManagedgrafanaError::ResourceAlreadyExistsException(format!("Workspace {} already exists", name)));
        }
        let arn = format!("arn:aws:grafana:{}:{}:workspaces/{}", state.region, state.account_id, name);
    ...
    #[allow(dead_code)]
    pub async fn get_workspace(&self, name: &str) -> Result<WorkspaceDetail, ManagedgrafanaError> {
        let state = self.inner.lock().await;
        let stored = state.workspaces.get(name)
            .ok_or_else(|| ManagedgrafanaError::ResourceNotFoundException(format!("Workspace {} not found", name)))?;
        Ok(WorkspaceDetail {
    ...
    #[allow(dead_code)]
    pub async fn list_workspaces(&self) -> Result<ListWorkspacesResponse, ManagedgrafanaError> {
        let state = self.inner.lock().await;
    ...
    #[allow(dead_code)]
    pub async fn delete_workspace(&self, name: &str) -> Result<(), ManagedgrafanaError> {
        let mut state = self.inner.lock().await;
...
mod tests {
    ...
    async fn test_new_state() {
        let _state = ManagedgrafanaState::new("123456789012".to_string(), "us-east-1".to_string());
    }
    #[tokio::test]
    async fn test_create_workspace() {
        let state = ManagedgrafanaState::new("123456789012".to_string(), "us-east-1".to_string());

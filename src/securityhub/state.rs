...
use super::error::SecurityhubError;
use super::types::*;

#[allow(dead_code)]
struct SecurityhubStateInner {
...
pub struct SecurityhubState {
    inner: Arc<Mutex<SecurityhubStateInner>>,
}
...
impl SecurityhubState {
    ...
    pub fn new(account_id: String, region: String) -> Self {
        SecurityhubState {
            inner: Arc::new(Mutex::new(SecurityhubStateInner {
    ...
    #[allow(dead_code)]
    pub async fn create_finding(&self, req: CreateFindingRequest) -> Result<FindingDetail, SecurityhubError> {
        let mut state = self.inner.lock().await;
        let name = req.name.or(req.name_pascal).unwrap_or_else(|| uuid::Uuid::new_v4().to_string());
        if state.findings.contains_key(&name) {
            return Err(SecurityhubError::ResourceAlreadyExistsException(format!("Finding {} already exists", name)));
        }
        let arn = format!("arn:aws:securityhub:{}:{}:findings/{}", state.region, state.account_id, name);
    ...
    #[allow(dead_code)]
    pub async fn get_finding(&self, name: &str) -> Result<FindingDetail, SecurityhubError> {
        let state = self.inner.lock().await;
        let stored = state.findings.get(name)
            .ok_or_else(|| SecurityhubError::ResourceNotFoundException(format!("Finding {} not found", name)))?;
        Ok(FindingDetail {
    ...
    #[allow(dead_code)]
    pub async fn list_findings(&self) -> Result<ListFindingsResponse, SecurityhubError> {
        let state = self.inner.lock().await;
    ...
    #[allow(dead_code)]
    pub async fn delete_finding(&self, name: &str) -> Result<(), SecurityhubError> {
        let mut state = self.inner.lock().await;
...
mod tests {
    ...
    async fn test_new_state() {
        let _state = SecurityhubState::new("123456789012".to_string(), "us-east-1".to_string());
    }
    #[tokio::test]
    async fn test_create_finding() {
        let state = SecurityhubState::new("123456789012".to_string(), "us-east-1".to_string());

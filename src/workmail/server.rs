...
async fn handle_request(
    State(state): State<Arc<WorkmailState>>,
    headers: HeaderMap,
    body: Bytes,
) -> Result<axum::response::Response, WorkmailError> {
    let target = headers
        .get("x-amz-target")
        .and_then(|v| v.to_str().ok())
        .ok_or_else(|| WorkmailError::InvalidAction("Missing X-Amz-Target header".into()))?;

    let action = target
        .strip_prefix("WorkMailService.")
        .ok_or_else(|| WorkmailError::InvalidAction(format!("Invalid target: {target}")))?;
    ...
    match action {
        ...
        _ => Err(WorkmailError::InvalidAction(format!("Unknown action: {action}"))),
    }
}

pub fn create_router(state: Arc<WorkmailState>) -> Router {
    Router::new()
...
mod tests {
    ...
    async fn test_missing_target_header() {
        let state = Arc::new(WorkmailState::new("123456789012".to_string(), "us-east-1".to_string()));
    ...
    async fn test_unknown_action() {
        ...
        let req = Request::builder()
            ...
            .uri("/")
            .header("content-type", "application/x-amz-json-1.1")
            .header("x-amz-target", "WorkMailService.FakeAction")
            .body(Body::from("{}"))
            .unwrap();
        let resp = app.oneshot(req).await.unwrap();
        assert_eq!(resp.status(), StatusCode::BAD_REQUEST);
    ...
    async fn test_describeorganization_requires_params() {
        ...
        let req = Request::builder()
            ...
            .uri("/")
            .header("content-type", "application/x-amz-json-1.1")
            .header("x-amz-target", "WorkMailService.DescribeOrganization")
            .body(Body::from("{}"))
            .unwrap();
        let resp = app.oneshot(req).await.unwrap();
    ...
    async fn test_createorganization_action() {
        ...
        let req = Request::builder()
            ...
            .uri("/")
            .header("content-type", "application/x-amz-json-1.1")
            .header("x-amz-target", "WorkMailService.CreateOrganization")
            .body(Body::from("{}"))
            .unwrap();
        let resp = app.oneshot(req).await.unwrap();
    ...
    async fn test_listorganizations_action() {
        ...
        let req = Request::builder()
            ...
            .uri("/")
            .header("content-type", "application/x-amz-json-1.1")
            .header("x-amz-target", "WorkMailService.ListOrganizations")
            .body(Body::from("{}"))
            .unwrap();
        let resp = app.oneshot(req).await.unwrap();

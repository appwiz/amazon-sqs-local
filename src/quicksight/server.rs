...
use super::state::QuicksightState;
use super::types::*;

async fn create_data_set_handler(
    State(state): State<Arc<QuicksightState>>,
    Json(req): Json<CreateDataSetRequest>,
) -> Result<axum::response::Response, QuicksightError> {
...
pub fn create_router(state: Arc<QuicksightState>) -> Router {
    Router::new()
...
mod tests {
    ...
    async fn test_list_endpoint() {
        let state = Arc::new(QuicksightState::new("123456789012".to_string(), "us-east-1".to_string()));

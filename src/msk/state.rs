...
impl MSKState {
    ...
    pub fn new(account_id: String, region: String) -> Self {
        MSKState {
            inner: Arc::new(Mutex::new(MSKStateInner {
    ...
    #[allow(dead_code)]
    pub async fn create_cluster(&self, req: CreateClusterRequest) -> Result<ClusterDetail, MSKError> {
        let mut state = self.inner.lock().await;
        let name = req.name.or(req.name_pascal).unwrap_or_else(|| uuid::Uuid::new_v4().to_string());
        if state.clusters.contains_key(&name) {
            return Err(MSKError::ResourceAlreadyExistsException(format!("Cluster {} already exists", name)));
        }
        let arn = format!("arn:aws:kafka:{}:{}:clusters/{}", state.region, state.account_id, name);
    ...
    #[allow(dead_code)]
    pub async fn get_cluster(&self, name: &str) -> Result<ClusterDetail, MSKError> {
        let state = self.inner.lock().await;
        let stored = state.clusters.get(name)
            .ok_or_else(|| MSKError::ResourceNotFoundException(format!("Cluster {} not found", name)))?;
        Ok(ClusterDetail {
    ...
    #[allow(dead_code)]
    pub async fn list_clusters(&self) -> Result<ListClustersResponse, MSKError> {
        let state = self.inner.lock().await;
    ...
    #[allow(dead_code)]
    pub async fn delete_cluster(&self, name: &str) -> Result<(), MSKError> {
        let mut state = self.inner.lock().await;
...
mod tests {
    ...
    async fn test_new_state() {
        let _state = MSKState::new("123456789012".to_string(), "us-east-1".to_string());
    }
    #[tokio::test]
    async fn test_create_cluster() {
        let state = MSKState::new("123456789012".to_string(), "us-east-1".to_string());

...
pub struct CodepipelineState {
    inner: Arc<Mutex<CodepipelineStateInner>>,
}
...
impl CodepipelineState {
    ...
    pub fn new(account_id: String, region: String) -> Self {
        CodepipelineState {
            inner: Arc::new(Mutex::new(CodepipelineStateInner {
    ...
    #[allow(dead_code)]
    pub async fn create_pipeline(&self, req: CreatePipelineRequest) -> Result<CreatePipelineResponse, CodepipelineError> {
        let mut state = self.inner.lock().await;
        let name = req.pipeline_name.clone();
        if state.pipelines.contains_key(&name) {
            return Err(CodepipelineError::ResourceAlreadyExistsException(format!("Pipeline {} already exists", name)));
        }
        let arn = format!("arn:aws:codepipeline:{}:{}:pipelines/{}", state.region, state.account_id, name);
    ...
    #[allow(dead_code)]
    pub async fn describe_pipeline(&self, req: DescribePipelineRequest) -> Result<DescribePipelineResponse, CodepipelineError> {
        let state = self.inner.lock().await;
        let name = req.pipeline_name.or_else(|| req.pipeline_arn.as_ref().and_then(|a| a.rsplit('/').next().map(|s| s.to_string())))
            .ok_or_else(|| CodepipelineError::ValidationException("Name or ARN required".to_string()))?;
        let stored = state.pipelines.get(&name)
            .ok_or_else(|| CodepipelineError::ResourceNotFoundException(format!("Pipeline {} not found", name)))?;
    ...
    #[allow(dead_code)]
    pub async fn list_pipelines(&self, _req: ListPipelinesRequest) -> Result<ListPipelinesResponse, CodepipelineError> {
        let state = self.inner.lock().await;
    ...
    #[allow(dead_code)]
    pub async fn delete_pipeline(&self, req: DeletePipelineRequest) -> Result<(), CodepipelineError> {
        let mut state = self.inner.lock().await;
        let name = req.pipeline_name.or_else(|| req.pipeline_arn.as_ref().and_then(|a| a.rsplit('/').next().map(|s| s.to_string())))
            .ok_or_else(|| CodepipelineError::ValidationException("Name or ARN required".to_string()))?;
...
mod tests {
    ...
    async fn test_new_state() {
        let _state = CodepipelineState::new("123456789012".to_string(), "us-east-1".to_string());
    }
    #[tokio::test]
    async fn test_create_pipeline() {
        let state = CodepipelineState::new("123456789012".to_string(), "us-east-1".to_string());

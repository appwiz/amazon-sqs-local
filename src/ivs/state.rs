...
impl IVSState {
    ...
    pub fn new(account_id: String, region: String) -> Self {
        IVSState {
            inner: Arc::new(Mutex::new(IVSStateInner {
    ...
    #[allow(dead_code)]
    pub async fn create_channel(&self, req: CreateChannelRequest) -> Result<ChannelDetail, IVSError> {
        let mut state = self.inner.lock().await;
        let name = req.name.or(req.name_pascal).unwrap_or_else(|| uuid::Uuid::new_v4().to_string());
        if state.channels.contains_key(&name) {
            return Err(IVSError::ResourceAlreadyExistsException(format!("Channel {} already exists", name)));
        }
        let arn = format!("arn:aws:ivs:{}:{}:channels/{}", state.region, state.account_id, name);
    ...
    #[allow(dead_code)]
    pub async fn get_channel(&self, name: &str) -> Result<ChannelDetail, IVSError> {
        let state = self.inner.lock().await;
        let stored = state.channels.get(name)
            .ok_or_else(|| IVSError::ResourceNotFoundException(format!("Channel {} not found", name)))?;
        Ok(ChannelDetail {
    ...
    #[allow(dead_code)]
    pub async fn list_channels(&self) -> Result<ListChannelsResponse, IVSError> {
        let state = self.inner.lock().await;
    ...
    #[allow(dead_code)]
    pub async fn delete_channel(&self, name: &str) -> Result<(), IVSError> {
        let mut state = self.inner.lock().await;
...
mod tests {
    ...
    async fn test_new_state() {
        let _state = IVSState::new("123456789012".to_string(), "us-east-1".to_string());
    }
    #[tokio::test]
    async fn test_create_channel() {
        let state = IVSState::new("123456789012".to_string(), "us-east-1".to_string());

mod _types {
use serde::{Deserialize, Serialize};

#[derive(Debug, Deserialize, Clone, Default)]
#[allow(dead_code)]
pub struct CreateCheckRequest {
    #[serde(rename = "CheckName")]
    pub check_name: String,
    #[serde(rename = "Tags")]
    #[serde(default)]
    pub tags: Option<std::collections::HashMap<String, String>>,
}

#[derive(Debug, Serialize)]
#[allow(dead_code)]
pub struct CreateCheckResponse {
    #[serde(rename = "CheckArn")]
    #[serde(skip_serializing_if = "Option::is_none")]
    pub check_arn: Option<String>,
    #[serde(rename = "CheckName")]
    #[serde(skip_serializing_if = "Option::is_none")]
    pub check_name: Option<String>,
}

#[derive(Debug, Deserialize, Clone, Default)]
#[allow(dead_code)]
pub struct DescribeCheckRequest {
    #[serde(rename = "CheckName")]
    pub check_name: Option<String>,
    #[serde(rename = "CheckArn")]
    pub check_arn: Option<String>,
}

#[derive(Debug, Serialize, Clone)]
#[allow(dead_code)]
pub struct CheckDetail {
    #[serde(rename = "CheckName")]
    pub check_name: String,
    #[serde(rename = "CheckArn")]
    pub check_arn: String,
    #[serde(rename = "Status")]
    pub status: String,
    #[serde(rename = "Tags")]
    #[serde(skip_serializing_if = "Option::is_none")]
    pub tags: Option<std::collections::HashMap<String, String>>,
}

#[derive(Debug, Serialize)]
#[allow(dead_code)]
pub struct DescribeCheckResponse {
    #[serde(rename = "Check")]
    pub check: CheckDetail,
}

#[derive(Debug, Deserialize, Clone, Default)]
#[allow(dead_code)]
pub struct ListChecksRequest {
    #[serde(rename = "MaxResults")]
    pub max_results: Option<usize>,
    #[serde(rename = "NextToken")]
    pub next_token: Option<String>,
}

#[derive(Debug, Serialize)]
#[allow(dead_code)]
pub struct ListChecksResponse {
    #[serde(rename = "Checks")]
    #[serde(skip_serializing_if = "Option::is_none")]
    pub checks: Option<Vec<CheckDetail>>,
    #[serde(rename = "NextToken")]
    #[serde(skip_serializing_if = "Option::is_none")]
    pub next_token: Option<String>,
}

#[derive(Debug, Deserialize, Clone, Default)]
#[allow(dead_code)]
pub struct DeleteCheckRequest {
    #[serde(rename = "CheckName")]
    pub check_name: Option<String>,
    #[serde(rename = "CheckArn")]
    pub check_arn: Option<String>,
}

}
pub use _types::*;

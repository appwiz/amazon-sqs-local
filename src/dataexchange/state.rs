...
pub struct DataexchangeState {
    inner: Arc<Mutex<DataexchangeStateInner>>,
}
...
impl DataexchangeState {
    ...
    pub fn new(account_id: String, region: String) -> Self {
        DataexchangeState {
            inner: Arc::new(Mutex::new(DataexchangeStateInner {
    ...
    #[allow(dead_code)]
    pub async fn create_data_set(&self, req: CreateDataSetRequest) -> Result<DataSetDetail, DataexchangeError> {
        ...
        if state.data_sets.contains_key(&name) {
            return Err(DataexchangeError::ResourceAlreadyExistsException(format!("DataSet {} already exists", name)));
        }
        let arn = format!("arn:aws:dataexchange:{}:{}:data-sets/{}", state.region, state.account_id, name);
    ...
    #[allow(dead_code)]
    pub async fn get_data_set(&self, name: &str) -> Result<DataSetDetail, DataexchangeError> {
        let state = self.inner.lock().await;
        let stored = state.data_sets.get(name)
            .ok_or_else(|| DataexchangeError::ResourceNotFoundException(format!("DataSet {} not found", name)))?;
        Ok(DataSetDetail {
    ...
    #[allow(dead_code)]
    pub async fn list_data_sets(&self) -> Result<ListDataSetsResponse, DataexchangeError> {
        let state = self.inner.lock().await;
    ...
    #[allow(dead_code)]
    pub async fn delete_data_set(&self, name: &str) -> Result<(), DataexchangeError> {
        let mut state = self.inner.lock().await;
...
mod tests {
    ...
    async fn test_new_state() {
        let _state = DataexchangeState::new("123456789012".to_string(), "us-east-1".to_string());
    }
    #[tokio::test]
    async fn test_create_data_set() {
        let state = DataexchangeState::new("123456789012".to_string(), "us-east-1".to_string());

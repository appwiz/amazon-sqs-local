...
fn require(params: &Params, key: &str) -> Result<String, NeptuneError> {
    params.get(key).cloned().ok_or_else(|| NeptuneError::ValidationException(format!("Missing required parameter: {}", key)))
}

fn xml_ok(action: &str, body: &str) -> Response {
    let xml = format!(
        r#"<{action}Response xmlns="https://neptune.amazonaws.com/doc/2012-10-01/">
  <{action}Result>
{body}
  </{action}Result>
  <ResponseMetadata>
...
async fn handle_request(
    State(state): State<Arc<NeptuneState>>,
    body: String,
    ...
    match action.as_str() {
        ...
        _ => Err(NeptuneError::InvalidAction(format!("Unknown action: {}", action))),
    }
}

pub fn create_router(state: Arc<NeptuneState>) -> Router {
    Router::new()
...
mod tests {
    ...
    async fn test_missing_action() {
        let state = Arc::new(NeptuneState::new("123456789012".to_string(), "us-east-1".to_string()));

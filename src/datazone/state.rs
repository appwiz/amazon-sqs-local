...
use super::error::DatazoneError;
use super::types::*;

#[allow(dead_code)]
struct DatazoneStateInner {
...
pub struct DatazoneState {
    inner: Arc<Mutex<DatazoneStateInner>>,
}
...
impl DatazoneState {
    ...
    pub fn new(account_id: String, region: String) -> Self {
        DatazoneState {
            inner: Arc::new(Mutex::new(DatazoneStateInner {
                domains: HashMap::new(),
                account_id,
                region,
            })),
    ...
    #[allow(dead_code)]
    pub async fn create_domain(&self, req: CreateDomainRequest) -> Result<DomainDetail, DatazoneError> {
        let mut state = self.inner.lock().await;
        let name = req.name.or(req.name_pascal).unwrap_or_else(|| uuid::Uuid::new_v4().to_string());
        if state.domains.contains_key(&name) {
            return Err(DatazoneError::ResourceAlreadyExistsException(format!("Domain {} already exists", name)));
        }
        let arn = format!("arn:aws:datazone:{}:{}:domains/{}", state.region, state.account_id, name);
    ...
    #[allow(dead_code)]
    pub async fn get_domain(&self, name: &str) -> Result<DomainDetail, DatazoneError> {
        let state = self.inner.lock().await;
        let stored = state.domains.get(name)
            .ok_or_else(|| DatazoneError::ResourceNotFoundException(format!("Domain {} not found", name)))?;
        Ok(DomainDetail {
    ...
    #[allow(dead_code)]
    pub async fn list_domains(&self) -> Result<ListDomainsResponse, DatazoneError> {
        let state = self.inner.lock().await;
    ...
    #[allow(dead_code)]
    pub async fn delete_domain(&self, name: &str) -> Result<(), DatazoneError> {
        let mut state = self.inner.lock().await;
...
mod tests {
    ...
    async fn test_new_state() {
        let _state = DatazoneState::new("123456789012".to_string(), "us-east-1".to_string());
    }
    #[tokio::test]
    async fn test_create_domain() {
        let state = DatazoneState::new("123456789012".to_string(), "us-east-1".to_string());

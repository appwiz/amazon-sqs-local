...
use super::error::AmplifyError;
use super::types::*;

#[allow(dead_code)]
struct AmplifyStateInner {
...
pub struct AmplifyState {
    inner: Arc<Mutex<AmplifyStateInner>>,
}
...
impl AmplifyState {
    ...
    pub fn new(account_id: String, region: String) -> Self {
        AmplifyState {
            inner: Arc::new(Mutex::new(AmplifyStateInner {
                apps: HashMap::new(),
                account_id,
                region,
            })),
    ...
    #[allow(dead_code)]
    pub async fn create_app(&self, req: CreateAppRequest) -> Result<AppDetail, AmplifyError> {
        let mut state = self.inner.lock().await;
        let name = req.name.or(req.name_pascal).unwrap_or_else(|| uuid::Uuid::new_v4().to_string());
        if state.apps.contains_key(&name) {
            return Err(AmplifyError::ResourceAlreadyExistsException(format!("App {} already exists", name)));
        }
        let arn = format!("arn:aws:amplify:{}:{}:apps/{}", state.region, state.account_id, name);
    ...
    #[allow(dead_code)]
    pub async fn get_app(&self, name: &str) -> Result<AppDetail, AmplifyError> {
        let state = self.inner.lock().await;
        let stored = state.apps.get(name)
            .ok_or_else(|| AmplifyError::ResourceNotFoundException(format!("App {} not found", name)))?;
        Ok(AppDetail {
    ...
    #[allow(dead_code)]
    pub async fn list_apps(&self) -> Result<ListAppsResponse, AmplifyError> {
        let state = self.inner.lock().await;
    ...
    #[allow(dead_code)]
    pub async fn delete_app(&self, name: &str) -> Result<(), AmplifyError> {
        let mut state = self.inner.lock().await;
...
mod tests {
    ...
    async fn test_new_state() {
        let _state = AmplifyState::new("123456789012".to_string(), "us-east-1".to_string());
    }
    #[tokio::test]
    async fn test_create_app() {
        let state = AmplifyState::new("123456789012".to_string(), "us-east-1".to_string());

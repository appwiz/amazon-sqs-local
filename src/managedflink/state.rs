...
use super::error::ManagedflinkError;
use super::types::*;

#[allow(dead_code)]
struct ManagedflinkStateInner {
...
#[allow(dead_code)]
pub struct ManagedflinkState {
    inner: Arc<Mutex<ManagedflinkStateInner>>,
}

impl ManagedflinkState {
    #[allow(dead_code)]
    pub fn new(account_id: String, region: String) -> Self {
        ManagedflinkState {
            inner: Arc::new(Mutex::new(ManagedflinkStateInner {
    ...
    #[allow(dead_code)]
    pub async fn create_application(&self, req: CreateApplicationRequest) -> Result<CreateApplicationResponse, ManagedflinkError> {
        let mut state = self.inner.lock().await;
        let name = req.application_name.clone();
        if state.applications.contains_key(&name) {
            return Err(ManagedflinkError::ResourceAlreadyExistsException(format!("Application {} already exists", name)));
        }
        let arn = format!("arn:aws:kinesisanalytics:{}:{}:applications/{}", state.region, state.account_id, name);
    ...
    #[allow(dead_code)]
    pub async fn describe_application(&self, req: DescribeApplicationRequest) -> Result<DescribeApplicationResponse, ManagedflinkError> {
        let state = self.inner.lock().await;
        let name = req.application_name.or_else(|| req.application_arn.as_ref().and_then(|a| a.rsplit('/').next().map(|s| s.to_string())))
            .ok_or_else(|| ManagedflinkError::ValidationException("Name or ARN required".to_string()))?;
        let stored = state.applications.get(&name)
            .ok_or_else(|| ManagedflinkError::ResourceNotFoundException(format!("Application {} not found", name)))?;
    ...
    #[allow(dead_code)]
    pub async fn list_applications(&self, _req: ListApplicationsRequest) -> Result<ListApplicationsResponse, ManagedflinkError> {
        let state = self.inner.lock().await;
    ...
    #[allow(dead_code)]
    pub async fn delete_application(&self, req: DeleteApplicationRequest) -> Result<(), ManagedflinkError> {
        let mut state = self.inner.lock().await;
        let name = req.application_name.or_else(|| req.application_arn.as_ref().and_then(|a| a.rsplit('/').next().map(|s| s.to_string())))
            .ok_or_else(|| ManagedflinkError::ValidationException("Name or ARN required".to_string()))?;
...
mod tests {
    ...
    async fn test_new_state() {
        let _state = ManagedflinkState::new("123456789012".to_string(), "us-east-1".to_string());
    }
    #[tokio::test]
    async fn test_create_application() {
        let state = ManagedflinkState::new("123456789012".to_string(), "us-east-1".to_string());
